...
namespace PayloadInstaller
{
    class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            if (mode == "uninstall")
            {
                // Read uninstall command and install location from registry
                ...
                // Run uninstall command from registry
                Console.WriteLine("Running uninstall command: " + uninstallString);
                int result = RunCommand(uninstallString, null);
            ...
            try
            {
                ...
                // Uninstall legacy packages first
                foreach (string pkg in packages)
                {
                    Console.WriteLine("\nUninstalling legacy product: " + pkg + "...");
                    RunCommand("qpm-cli", "--uninstall " + pkg);
                }
                ...
                // On success, clean installPath and copy temp contents
                if (result == 0)
                {
...
$sourceFile = Join-Path $PSScriptRoot "QUD_Installer.cs"
