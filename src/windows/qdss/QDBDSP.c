...
)
{
    ...
    else
    {
        ...
        (
            QDB_DBG_MASK_CONTROL,
            QDB_DBG_LEVEL_DETAIL,
            ("<%s> QDBDSP_IoDeviceControl: [Unknown - 0x%x]\n", pDevContext->PortName, fileContext->Type)
        );
    }
    ...
    switch (IoControlCode)
    {
        case IOCTL_QCDEV_DPL_STATS:
        {
            ...
            ntStatus = QDBDSP_GetDPLStats(pDevContext, Request);
            break;
        }
        ...
        case IOCTL_QCDEV_REQUEST_DEVICEID:
        {
            ...
            (
                QDB_DBG_MASK_CONTROL,
                QDB_DBG_LEVEL_DETAIL,
                ("<%s> QDBDSP_IoDeviceControl: IOCTL_QCDEV_REQUEST_DEVICEID (IN/OUT %d/%d)\n",
                pDevContext->PortName, InputBufferLength, OutputBufferLength)
            );
            ntStatus = QDBDSP_GetParentId(pDevContext, Request);
            break;
        }
...
/****************************************************************************
 *
 * function: QDBDSP_GetDPLStats
 *
 * purpose:  Copy the QDB_STATS data from device context to IOCTL request
 *
 * arguments:pDevContext = pointer to the device context
 *           Request     = input IOCTL request
 *
 * returns:  NT status
 *
 ****************************************************************************/
...
)
{
    size_t   length = 0;
    PVOID    buffer = NULL;
    NTSTATUS status = WdfRequestRetrieveOutputBuffer(Request, sizeof(QDB_STATS), &buffer, &length);

    if (NT_SUCCESS(status))
    {
        if (length < sizeof(QDB_STATS))
        {
            status = STATUS_BUFFER_TOO_SMALL;
        }
        else
        {
            RtlCopyMemory(buffer, &(pDevContext->Stats), sizeof(QDB_STATS));
 ...
 *
 * purpose:  Forward the IOCTL_QCDEV_REQUEST_DEVICEID IOCTL to the
 *           lower-level USB composite driver
 *
 * arguments:pDevContext = pointer to the device context
 *           Request     = IOCTL request to forward
 *
 * returns:  NT status
 *
 ****************************************************************************/
...
)
{
    ...
    if (WdfRequestSend(Request, pDevContext->MyIoTarget, &options) == FALSE)
    {
