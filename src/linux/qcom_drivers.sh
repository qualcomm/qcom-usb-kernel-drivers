...
# Persist dependency modules across reboots on Debian (and Ubuntu) using modules-load.d
if [[ $OSName =~ "Debian" ]] || [[ $OSName =~ "Ubuntu" ]]; then
   QCOM_MODULES_LOAD_CONF=/etc/modules-load.d/qcom-qud.conf
   echo -e "Writing dependency modules to $QCOM_MODULES_LOAD_CONF for boot persistence"
   cat > $QCOM_MODULES_LOAD_CONF << 'MODULES_EOF'
# QUD dependency modules - auto-generated by qcom_drivers.sh
usbserial
mii
usbnet
MODULES_EOF
   $QCOM_LN_RM_MK_DIR/chmod 644 $QCOM_MODULES_LOAD_CONF
   echo -e "Dependency modules registered in $QCOM_MODULES_LOAD_CONF"
fi
